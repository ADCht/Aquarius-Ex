...
AQUAENTRY(void, InitChildProcessA)(LPVOID conf, LPCSTR value) {
	aqua_convert conv;
	CefString(&reinterpret_cast<CefSettings*>(conf)->browser_subprocess_path) = conv.from_bytes(value);
}
...
AQUAENTRY(void, InitChildProcessW)(LPVOID conf, LPCWSTR value) {
	CefString(&reinterpret_cast<CefSettings*>(conf)->browser_subprocess_path) = value;
}
...
AQUAENTRY(void, InitSingleProcess)(LPVOID conf, BOOL enabled) {
	reinterpret_cast<CefSettings*>(conf)->single_process = enabled;
}
...
AQUAENTRY(void, InitMultiThreadedMessageLoop)(LPVOID conf, BOOL enabled) {
	reinterpret_cast<CefSettings*>(conf)->multi_threaded_message_loop = enabled;
}
...
///
AQUAENTRY(void, EnableHighDPISupport)() {
